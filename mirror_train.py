...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if gaussians.checkpoint_mirror_transform is not None:
            image = image * (1 - gt_mirror_mask) 

        Ll1 = l1_loss(image, gt_image_wo_mirror)
            
        if FUSED_SSIM_AVAILABLE:
            ssim_value = fused_ssim(image.unsqueeze(0), gt_image_wo_mirror.unsqueeze(0))
        else:
            ssim_value = ssim(image, gt_image_wo_mirror)
        ...
        if iteration > opt.first_stage_iter and viewpoint_cam.gt_alpha_mask.sum() > 0: # stage2: fuse image 
            ...
            mirror_render_pkg = render(viewpoint_cam, gaussians, pipe, background, use_mirror_transform=True)
            ...
            Ll1_mirror = 2.0 * l1_loss(mirror_image * gt_mirror_mask, gt_image * gt_mirror_mask)
            # ssim_value_mirror = ssim(mirror_image, gt_image * gt_mirror_mask)
            # loss += (1.0 - opt.lambda_dssim) * Ll1_mirror + opt.lambda_dssim * (1.0 - ssim_value_mirror)
            loss +=Ll1_mirror
        ...
        with torch.no_grad():
            ...
            if (iteration in checkpoint_iterations or iteration == opt.first_stage_iter):
                ...
                if iteration==opt.first_stage_iter:
                    ...
                    torch.save((gaussians.capture(), iteration), scene.model_path + "/chkpnt_stage_2_" + str(iteration) + ".pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[30_000, 40000])
